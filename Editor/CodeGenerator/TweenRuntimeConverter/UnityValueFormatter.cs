    ...
    public static class UnityValueFormatter
    {
        public static string FormatValue(object obj)
        {
            ...
            switch (type)
            {
                ...
                default:
                    return value;
                    // throw new IndexOutOfRangeException($"Can not FormatValue {value}");
            }
        }
        ...
        private static string FormatVector2Int(string value)
        {
            // Expected format: "x, y"
            var parts = value.Trim('(', ')').Split(',');
            if (parts.Length == 2)
            {
                return $"new Vector2Int({parts[0].Trim()}, {parts[1].Trim()})";
            }
            return value; // Return the original value if format is not as expected
        }
