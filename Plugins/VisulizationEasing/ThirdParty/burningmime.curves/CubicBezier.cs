...
namespace burningmime.curves
{
    ...
    public struct CubicBezier : IEquatable<CubicBezier>
    {
        ...
        /// <summary>
        /// Evaluates the cubic bezier curve and returns the interpolated value.
        /// </summary>
        /// <param name="t">Current time in seconds.</param>
        /// <param name="b">Starting value.</param>
        /// <param name="c">Change in value.</param>
        /// <param name="d">Duration of animation.</param>
        /// <returns>The interpolated value based on the Bezier curve.</returns>
        public double Evaluate(double t, double b, double c, double d)
        {
            // Normalize time t to the range [0, 1]
            double normalizedT = t / d;

            // Sample the Bezier curve
            var sampledPoint = Sample((float)normalizedT);

            // The y-component of the sampled point represents the interpolation factor
            double bezierValue = sampledPoint.y;

            // Calculate the final interpolated value based on the starting value, change, and Bezier curve result
            return b + bezierValue * c;
        }
